...
if __name__ == "__main__":
    ...
    # 모델 외부거 다운받아서 하기
    # config = AutoConfig.from_pretrained("beomi/KoAlpaca-Polyglot-5.8B")
    # config.temperature = 0.3
    # config.do_sample=True
    
    model = AutoModelForCausalLM.from_pretrained("beomi/KoAlpaca-Polyglot-5.8B", trust_remote_code=True, torch_dtype=torch.float16)

    tokenizer = AutoTokenizer.from_pretrained("beomi/KoAlpaca-Polyglot-5.8B", trust_remote_code=True)
    ...
    for i, test_data in enumerate(tqdm(test_dataset)): 
        ...
        try: 
            ...
            # answer = int(response.split(args.response_split)[1].strip())
            # id_list.append(i)
